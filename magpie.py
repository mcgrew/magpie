...
def parseOpts():
  ...
  parser.add_option("-f", "--file", dest="file",
    default=os.path.expanduser(os.path.join('~', ".config", "magpie", "database")),
...
#    "result as the new database.")
  ...
  parser.add_option("-s", "--salt", dest="saltfile", metavar="FILE",
    default=os.path.expanduser(os.path.join('~', ".config", "magpie", "salt")),
...
#         raise ValueError
  ...
  def flush(self) -> None:
    ...
    with open(self.filename, 'wb', 0o600) as passFile:
      passFile.write(b64encode(self.encrypt(zlib.compress(self.data.encode('utf-8')[::-1], 9))))
  ...
  def open(self) -> bool:
    ...
    with open(self.filename, 'rb') as passFile:
      self.data =  zlib.decompress(self.decrypt(b64decode(passFile.read())))[::-1].decode('utf-8')
    return True
